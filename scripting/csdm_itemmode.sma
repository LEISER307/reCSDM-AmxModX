...
new PLUGINNAME[] = "ReCSDM Item Mode";
...
public use_csdm_item_menu(id, menu, item)
{
	...
	if (!menu_item_getinfo(g_ItemSettMenu, item, paccess, command, charsmax(command), _, 0, call)) {
	...
	if (equali(command,"i_m_change_state"))
	{
		...
		if (g_droppacks > 2) g_droppacks = 0;
		...
		num_to_str(g_droppacks, sz, charsmax(sz));
...
public c_ItemMode(id, menu, item)
{
	...
	if (g_Enabled) {
		formatex(fItem, charsmax(fItem),"Item Mode Enabled");
		menu_item_setname(menu, item, fItem);
		return ITEM_ENABLED;

	} else if (!g_EntCount) {
		formatex(fItem, charsmax(fItem),"Item Mode Disabled - no items");
		menu_item_setname(menu, item, fItem);
		return ITEM_DISABLED;

	} else {
		formatex(fItem, charsmax(fItem),"Item Mode Disabled");
		menu_item_setname(menu, item, fItem);
		return ITEM_ENABLED;
	}
}
...
public c_PersItems(id, menu, item)
{
	...
	if (g_bPersistentItems) {
		formatex(fItem, charsmax(fItem),"Items Persistent");
		menu_item_setname(menu, item, fItem);
		return ITEM_ENABLED;
	} else {
		formatex(fItem, charsmax(fItem),"Items Falling Down");
		menu_item_setname(menu, item, fItem);
		return ITEM_ENABLED;
	}
}
...
public c_NoGunMenu(id, menu, item)
{
	...
	if (g_bNoGunMenu) {
		formatex(fItem, charsmax(fItem),"Gun Menu Disabled for Item Mode");
		menu_item_setname(menu, item, fItem);
		return ITEM_ENABLED;
	} else {
		formatex(fItem, charsmax(fItem),"Gun Menu Enabled for Item Mode");
		menu_item_setname(menu, item, fItem);
		return ITEM_ENABLED;
	}
}
...
public cfgrestricts(readAction, line[], section[])
{
	if (readAction == CFG_READ)
	{
		...
		parse(line, itemname, charsmax(itemname));
 ...
 // SL: we need to read 'weapons_stay' value only
public cfgsettings(readAction, line[], section[])
{
	if (readAction == CFG_READ)
	{
		new setting[24], sign[3], value[32];
...
ReadFile()
{
	...
	formatex(File, charsmax(File), "%s\csdm\items\ents_%s.cfg", config, Map);

	g_EntCount = 0;

	if (file_exists(File))
	{
		new Data[51], len, EntName[25], line, pos[4][8], iParamCount, iTotalItems = 0;

		while (g_EntCount < MAX_ITEMS && ((line = read_file(File, line, Data, charsmax(Data), len)) != 0))
		{
			if (strlen(Data) < 2) {
				continue;
			}
			...
			iParamCount = parse(Data, EntName, charsmax(EntName), pos[0], 7, pos[1], 7, pos[2], 7, pos[3], 7);

			if(iParamCount < 4) {
				continue;
			}

			parse(Data, EntName, charsmax(EntName), pos[0], 7, pos[1], 7, pos[2], 7);

			g_EntVecs[g_EntCount][0] = str_to_num(pos[0]);
			g_EntVecs[g_EntCount][1] = str_to_num(pos[1]);
			g_EntVecs[g_EntCount][2] = str_to_num(pos[2]);

			// Simon Logic: store item angle (yaw) - 5th alternative value
			if(iParamCount == 5) {
...
public removeRespawnableItem(id)
{
	if (!g_bPersistentItems)
	{
		...
		new arrParams[3], Float:vecOrigin[3];
...
{
	static item_type, ammo_via_offset;

	if (!g_Enabled || g_EntTable[ptr] < 0 || ptd < 1 || ptd > g_MaxPlayers
		|| !csdm_active() || !is_user_connected(ptd))
	{
	...
	// ignore entities with "wrong" physics
	if(!(pev(ptr, pev_movetype) & MOVETYPE_TOSS))
		return FMRES_IGNORED;

	// ignore invisible entities
	if(pev(ptr, pev_effects) & EF_NODRAW)
		return FMRES_IGNORED;

	 // ignore elevators, walls etc.
	if(pev(ptr, pev_solid) & SOLID_BSP)
		return FMRES_IGNORED;

	// ignore dead players
	if(pev(ptd, pev_deadflag) != DEAD_NO)
		return FMRES_IGNORED;
			...
			for (new i = SLOTS_AUX; i < maxiter; i++)
			{
				...
				if(CanGetWeapon(ptd, weap, ammo_via_offset))
				{
					if(ammo_via_offset) {
						...
						get_weaponname(weap, weapname, charsmax(weapname));
	...
	switch(item_type)
	{
		case ITEM_PISTOLAMMO..ITEM_PARAAMMO:
		{
			// Ammo
		...
		case 1..30:
		{
			...
			if(CanGetWeapon(ptd, item_type, ammo_via_offset))
			{
				...
				get_weaponname(item_type, weapname, charsmax(weapname));
		...
		{
			// Full Ammo
			if (CanGetAmmo(ptd, ITEM_PISTOLAMMO) || CanGetAmmo(ptd, ITEM_RIFLEAMMO) || CanGetAmmo(ptd, ITEM_SHOTAMMO)
				|| CanGetAmmo(ptd, ITEM_SMGAMMO) || CanGetAmmo(ptd, ITEM_AWPAMMO) || CanGetAmmo(ptd, ITEM_PARAAMMO))
			{
...
// arr = [ent_id, attempt, set_bbox]
public taskMaterialize(arr[3])
{
	// NO STATIC VARS!
	new id;
...
{
	if (!g_Enabled || !g_droppacks || !csdm_active()) return;
...
bool:CanGetAmmo(id, ammotype)
{
	switch (ammotype)
	{
		...
		{
			if (cs_get_user_bpammo(id, CSW_DEAGLE) < g_MaxBPAmmo[CSW_DEAGLE]) return true;
			if (cs_get_user_bpammo(id, CSW_P228) < g_MaxBPAmmo[CSW_P228]) return true;
			if (cs_get_user_bpammo(id, CSW_USP) < g_MaxBPAmmo[CSW_USP]) return true;
			if (cs_get_user_bpammo(id, CSW_GLOCK18) < g_MaxBPAmmo[CSW_GLOCK18]) return true;
			if (cs_get_user_bpammo(id, CSW_FIVESEVEN) < g_MaxBPAmmo[CSW_FIVESEVEN]) return true;
		}
		case ITEM_SHOTAMMO:
		{
			if (cs_get_user_bpammo(id, CSW_XM1014) < g_MaxBPAmmo[CSW_XM1014]) return true;
		}
		case ITEM_RIFLEAMMO:
		{
			if (cs_get_user_bpammo(id, CSW_M4A1) < g_MaxBPAmmo[CSW_M4A1]) return true;
			if (cs_get_user_bpammo(id, CSW_AK47) < g_MaxBPAmmo[CSW_AK47]) return true;
		}
		case ITEM_SMGAMMO:
		{
			if (cs_get_user_bpammo(id, CSW_MP5NAVY) < g_MaxBPAmmo[CSW_MP5NAVY]) return true;
			if (cs_get_user_bpammo(id, CSW_MAC10) < g_MaxBPAmmo[CSW_MAC10]) return true;
			if (cs_get_user_bpammo(id, CSW_P90) < g_MaxBPAmmo[CSW_P90]) return true;
		}
		case ITEM_AWPAMMO:
		{
			if (cs_get_user_bpammo(id, CSW_AWP) < g_MaxBPAmmo[CSW_AWP]) return true;
		}
		case ITEM_PARAAMMO:
		{
			if (cs_get_user_bpammo(id, CSW_M249) < g_MaxBPAmmo[CSW_M249]) return true;
		}
	}
...
bool:CanGetWeapon(id, wid, &ammo_via_offset)
{
	...
	if(slot == 4)
	{
		...
		ammo = cs_get_user_bpammo(id, wid);

		switch(wid)
		{
			case CSW_HEGRENADE:
			{
				if(ammo >= g_iMaxNadesH) return false;
				if(ammo >= g_MaxBPAmmo[CSW_HEGRENADE]) ammo_via_offset = ammo + 1;
			}
			case CSW_SMOKEGRENADE:
			{
				if(ammo >= g_iMaxNadesS) return false;
				if(ammo >= g_MaxBPAmmo[CSW_SMOKEGRENADE]) ammo_via_offset = ammo + 1;
			}
			case CSW_FLASHBANG:
			{
				if(ammo >= g_iMaxNadesF) return false;
				if(ammo >= g_MaxBPAmmo[CSW_FLASHBANG]) ammo_via_offset = ammo + 1;
			}
		}
		...
		for(new i = 0; i < num; i++)
		{
			...
			if (g_WeaponSlots[Weapons[i]] == slot) return false;
...
#endif
	if(type <= 30 && !isGrenadeItem(type)) { // it's a weapon
...
// actually is a weaponbox
stock bool:isWeapon(ent_id)
{
	static sBuffer[11]; // sizeof("weaponbox") + 1

	pev(ent_id, pev_classname, sBuffer, charsmax(sBuffer));
...
// grenades supported only
stock giveWeaponAgain(ptd, item_id, ammo)
{
#if defined _ASSERT	
	assert 0 <= item_id <= 30;
#endif
...
public m_MainHandler(id, menu, item)
{
	...
	{
		case 1: menu_display(id, g_AddItemsMenuID, 0);
		case 2:
		{
			new Float:vecs[3], vec[3], Float:angles[3], angle[3], angle_y;
		...
		{	
			...
			client_print(id, print_chat,"Total Items: %d;^nCurrent Origin: X: %f  Y: %f  Z: %f", g_EntCount, Org[0], Org[1], Org[2]);
	...
	switch(str_to_num(cmd))
	{
		case 1:
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Items - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;
			} else {
				formatex(fItem, charsmax(fItem),"Add a CSDM Item");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount < 1) {
				formatex(fItem, charsmax(fItem),"Edit Items - No items");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else if (g_Ent[id] == -1) {
				formatex(fItem, charsmax(fItem),"Edit Item - No item marked");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Edit closest Item (yellow) to Current Position");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount < 1) {
				formatex(fItem, charsmax(fItem),"Delete Item - No items");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else if (g_Ent[id] == -1) {
				formatex(fItem, charsmax(fItem),"Delete Item - No item marked");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;
				...
				if (distance > 200) {
					formatex(fItem, charsmax(fItem),"Delete Item - Marked item far away");
					menu_item_setname(menu, item, fItem);
					return ITEM_DISABLED;

				} else {
					formatex(fItem, charsmax(fItem),"Delete closest Item");
					menu_item_setname(menu, item, fItem);
					return ITEM_ENABLED;
				}
			}
...
public m_AddItemsHandler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: menu_display(id, g_AddPistolsMenuID, 0);
		case 2: menu_display(id, g_AddSmgMenuID, 0);
		case 3: menu_display(id, g_AddRifles1MenuID, 0);
		case 4: menu_display(id, g_AddRifles2MenuID, 0);
		case 5: menu_display(id, g_AddShotgunMenuID, 0);
		case 6: menu_display(id, g_AddEquipMenuID, 0);
	}
...
public c_AddItems(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Pistol - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Pistol");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Sub Machine Gun - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Sub Machine Gun");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Rifle (1) - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Rifle (1)");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Rifle (2) - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Rifle (2)");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Shotgun/Machine gun - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Shotgun/Machine gun");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add Equipement - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Equipement");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
public m_AddPistolsHandler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: add_item(vec, "weapon_p228", angle_y);
		case 2: add_item(vec, "weapon_usp", angle_y);
		case 3: add_item(vec, "weapon_elite", angle_y);
		case 4: add_item(vec, "weapon_fiveseven", angle_y);
		case 5: add_item(vec, "weapon_p228", angle_y);
		case 6: add_item(vec, "weapon_deagle", angle_y);
		case 7: add_item(vec, "pistol_ammo", angle_y);
	}
...
public c_AddPistols(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add glock18 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add glock18");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add usp - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add usp");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add elite - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add elite");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add fiveseven - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add fiveseven");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add p228 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add p228");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add deagle - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add deagle");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add pistol ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add pistol ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
public m_AddSmgHandler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: add_item(vec, "weapon_p90", angle_y);
		case 2: add_item(vec, "weapon_tmp", angle_y);
		case 3: add_item(vec, "weapon_ump45", angle_y);
		case 4: add_item(vec, "weapon_mac10", angle_y);
		case 5: add_item(vec, "weapon_mp5navy", angle_y);
		case 6: add_item(vec, "smg_ammo", angle_y);
	}
...
public c_AddSmg(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add p90 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add p90");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add tmp - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add tmp");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add ump45 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add ump45");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add mac10 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add mac10");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add mp5navy - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add mp5navy");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add smg ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add smg ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
public m_AddRifles1Handler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: add_item(vec, "weapon_ak47", angle_y);
		case 2: add_item(vec, "weapon_m4a1", angle_y);
		case 3: add_item(vec, "weapon_aug", angle_y);
		case 4: add_item(vec, "weapon_sg552", angle_y);
		case 5: add_item(vec, "weapon_scout", angle_y);
		case 6: add_item(vec, "weapon_sg550", angle_y);
		case 7: add_item(vec, "rifle_ammo", angle_y);
	}
...
public c_AddRifles1(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add ak47 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add ak47");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add m4a1 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add m4a1");
				menu_item_setname(menu, item, fItem );
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add aug - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add aug");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add sg552 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add sg552");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add scout - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add scout");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add sg550 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add sg550");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add rifle ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add rifle ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
public m_AddRifles2Handler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: add_item(vec, "weapon_g3sg1", angle_y);
		case 2: add_item(vec, "weapon_galil", angle_y);
		case 3: add_item(vec, "weapon_famas", angle_y);
		case 4: add_item(vec, "rifle_ammo", angle_y);
		case 5: add_item(vec, "weapon_awp", angle_y);
		case 6: add_item(vec, "awp_ammo", angle_y);
	}
...
public c_AddRifles2(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add g3sg1 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add g3sg1");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add galil - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add galil");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add famas - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add famas");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add awp - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add awp");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add awp ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add awp ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
public m_AddShotgunHandler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: add_item(vec, "weapon_xm1014", angle_y);
		case 2: add_item(vec, "weapon_m3", angle_y);
		case 3: add_item(vec, "shotgun_ammo", angle_y);
		case 4: add_item(vec, "weapon_m249", angle_y);
		case 5: add_item(vec, "para_ammo", angle_y);
	}
...
public c_AddShotgun(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add xm1014 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add xm1014");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add m3 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add m3");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add shotgun ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add shotgun ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add m249 - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add m249");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add para ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add para ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
public m_AddEquipHandler(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1: add_item(vec, "item_longjump", angle_y);
		case 2: add_item(vec, "item_healthkit", angle_y);
		case 3: add_item(vec, "armor", angle_y);
		case 4: add_item(vec, "item_battery", angle_y);
		case 5: add_item(vec, "full_ammo", angle_y);
		case 6: add_item(vec, "weapon_hegrenade", angle_y);
		case 7: add_item(vec, "weapon_flashbang", angle_y);
		case 8: add_item(vec, "weapon_smokegrenade", angle_y);
	}
...
public c_AddEquip(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add longjump - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add longjump");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add healthkit - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add healthkit");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add armor - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add armor");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add battery - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add battery");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add full ammo - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add full ammo");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add hegrenade - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add hegrenade");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add flashbang - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add flashbang");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_EntCount >= MAX_ITEMS) {
				formatex(fItem, charsmax(fItem),"Add smokegrenade - Max Items Limit Reached");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add smokegrenade");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
...
add_item(vec[3], EntName[25], angle = 0)
{
	...
	formatex(MapFile, charsmax(MapFile), "%s\csdm\items\ents_%s.cfg", config, Map);

	if(angle) {
		formatex(line, charsmax(line), "%s %d %d %d %d", EntName, vec[0], vec[1], vec[2], angle);
	} else {
		formatex(line, charsmax(line), "%s %d %d %d", EntName, vec[0], vec[1], vec[2]);
	}
	...
	if (CWRAP(EntName, "item_longjump")) g_EntType[g_EntCount] = ITEM_LONGJUMP;
	else if (CWRAP(EntName, "item_healthkit")) g_EntType[g_EntCount] = ITEM_MEDKIT;
	else if (CWRAP(EntName, "item_battery")) g_EntType[g_EntCount] = ITEM_BATTERY;
	else if (CWRAP(EntName, "pistol_ammo")) g_EntType[g_EntCount] = ITEM_PISTOLAMMO;
	else if (CWRAP(EntName, "rifle_ammo")) g_EntType[g_EntCount] = ITEM_RIFLEAMMO;
	else if (CWRAP(EntName, "shotgun_ammo")) g_EntType[g_EntCount] = ITEM_SHOTAMMO;
	else if (CWRAP(EntName, "smg_ammo")) g_EntType[g_EntCount] = ITEM_SMGAMMO;
	else if (CWRAP(EntName, "full_ammo")) g_EntType[g_EntCount] = ITEM_FULLAMMO;
	else if (CWRAP(EntName, "armor")) g_EntType[g_EntCount] = ITEM_ARMOR;
	else if (CWRAP(EntName, "awp_ammo")) g_EntType[g_EntCount] = ITEM_AWPAMMO;
	else if (CWRAP(EntName, "para_ammo")) g_EntType[g_EntCount] = ITEM_PARAAMMO;
	else {
		new weaptype = getWeapId(EntName);
...
edit_item(vec[3], ent, angle=0)
{
	...
	{
		new Data[124], len, line, pos[3][8], currentVec[3], newItem[128], EntName[25];
		...
		while ((line = read_file(MapFile, line, Data, charsmax(Data), len)) != 0)
		{
			...
			if (g_EntVecs[ent][0] == currentVec[0] && g_EntVecs[ent][1] == currentVec[1] && (g_EntVecs[ent][2] - currentVec[2]) <= 20) {
				...
				if(angle) {
					formatex(newItem, charsmax(newItem), "%s %d %d %d %d", EntName, vec[0], vec[1], vec[2], angle);
				} else {
					formatex(newItem, charsmax(newItem), "%s %d %d %d", EntName, vec[0], vec[1], vec[2]);
				}
...
delete_item(ent)
{
	...
	if (file_exists(MapFile))
	{
		new Data[124], len, line, pos[3][8], currentVec[3], EntName[25];
...
closest_item(id)
{
	...
	new origin[3], closest, lastDist = 999999;
...
public csdm_entdump(id)
{
	...
	new i, model[32], owner, class[32], Text[128], entid;
	// new pid, did
	...
	for (i = 0; i < g_EntCount; i++)
	{
		...
		if (pev_valid(entid)) 
		{
			pev(entid, pev_classname, class, charsmax(class));
			...
			formatex(Text, charsmax(Text), "[%d] (%d, %d) ^"%s^" ^"%s^"", i, entid, owner,
			// pid, did, 
			class, model);
